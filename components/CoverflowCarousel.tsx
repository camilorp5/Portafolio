...
export function CoverflowCarousel({
  ...
  falloff = 0.56,
  fade = 0.1,
  cardWidth = "clamp(220px, 30vw, 320px)", // Ligero ajuste para ver mejor las tarjetas
  gap = 0.05,
  loop = true,
  showCaption = true,
  showPagination = true,
          ...
          {active.meta && active.meta.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2 justify-center">
              {active.meta.map((row) => (
                <span
                  key={row.label}
                  className="rounded border border-blue-200 bg-blue-50 px-2.5 py-0.5 text-xs font-semibold text-blue-700"
                >
                  {row.value}
                </span>
              ))}
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          {slides.map((_, index) => (
            <button
              ...
              onClick={() => goTo(index)}
              className={cn(
                "size-2.5 rounded-full bg-blue-600 transition-opacity",
